...
\notecarl{En Process kan ikke oversættes med async. Det i beskriver er dog funktionalitet som StateProcess med OnTickAsync, som godt kan oversættes}
class provides
...
\begin{description}
  \item[\csharpextend{int AllocateSegment(int size)}]\hfill\\
  ...
  smaller than \csharpextend{int size}, -1 is returned, indicating
  error. If not, the segment table id is
  returned(\csharpextend{int seg_ID}).

  \item[\csharpextend{int FocusSegment()}]\hfill\\
  ...
  \item[\csharpextend{int SaveData(int seg_ID, int offset)}]\hfill
  ...
  \item[\csharpextend{int LoadData(int seg_ID, int offset)}]\hfill
  The same as \csharpextend{int SaveData(...)}, with the exception
  that an error is returned if we are not in the loading state.

  \item[\csharpextend{int DelaySegment(int seg_ID)}]\hfill\\
  This function delays a focus segment by copying it to the table head, and
  freeing the current \csharpextend{int seg_ID}. When this happens, the
  ...
  \item[{\pbox[c][1.7\baselineskip][b]{\linewidth}{{\csharpextend{void SaveMetaData(int seg_ID, MetaData meta)}}}}]\hfill

  Saves the \csharpextend{MetaData} into the current segment.

  \item[\csharpextend{MetaData LoadMetaData(int seg_ID)}]\hfill\\
  Loads the \csharpextend{MetaData} from the current segment.

  \item[\csharpextend{void SegmentFull(int seg_ID)}]\hfill\\
  Returns if the segment is \texttt{Full}.

  \item[\csharpextend{void SegmentDone(int seg_ID)}]\hfill\\
  ...
  Returns the \csharpextend{int seg_ID} if a space is available.
...
The single segments does not return an \csharpextend{int seg_ID} at any point,
...
\begin{description}
  \item[\csharpextend{bool New(int key)}]\hfill\\
  ...
  \item[\csharpextend{bool Free(int key)}]\hfill\\
  ...
  \item[\csharpextend{bool ContainsKey(int key)}]\hfill\\
  Test if the key table have the key.

  \item[\csharpextend{int GetFirstValue(int key)}]\hfill\\
  ...
  \item[\csharpextend{int ListLength(int key)}]\hfill\\
  ...
  \autoref{fig:memory_dictionary_explained}, the length of the list in
		key "0" is 10\notemark{Markeret af Carl. Jeg kan hellere ikke
		helt se det fra tabellen...};

  \item[\csharpextend{int Insert(int key, int index)}]\hfill\\
  ...
  \item[\csharpextend{int Delete(int key, int index)}]\hfill\\
  ...
  \item[\csharpextend{int Observe(int key, int index)}]\hfill\\
  Observe just returns the address if it exists, or -1 if not.

  \item[{\pbox[c][1.7\baselineskip][b]{\linewidth}{{\csharpextend{void SaveMetaData(int key, MetaData meta_data)}}}}]\hfill

  The keys have space for metadata, that can be loaded and saved.

  \item[\csharpextend{MetaData LoadMetaData(int key)}]\hfill\\
  Load the metadata.
...
low(or high)\notemark{Hvorfor begge? Betyder det noget?} throughput and high capacity. The throughput would have to be equal
...
end of a packet\notejan{AXI4-Stream has TLAST} (or simply just a segment of data), and the beginning of a new
...
\texttt{bytes\_left}\notejan{TUSER kan maaske have bytes left?}
\notecarl{Kan axi stream ikke også have den her? Må jeg lige få læst op på}
...
rules\notejan{Vi har genopfundet AXI4-Stream}:
\begin{itemize}
