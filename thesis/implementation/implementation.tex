...
Out}).\\
The Buffer-Producer is identical to the Transfer signalling of the
AXI4-Stream protocol, which ensures a two-way flow-control mechanism for both
...
In the Buffer-Producer protocol, as well as the AXI4, only a \texttt{valid}
flag is needed on the producer, and a \texttt{ready} on the consumer. With such
a few flags required during the data transfer, simple rules are defined for the
BP protocol, inspired by the AXI4 handshake process.
...
This data transfer process is visualized on figure \ref{fig:axi4_handshake},
where the \texttt{INFORMATION} along with \texttt{TVALID} is raised by the
producer, while the \texttt{TREADY} is raised one clock after where the the
data-exchange happens.
The conventional data-exchange in the network stack is perhaps better
visualized by a sequence diagram on figure \ref{fig:buffer_producer}.
...
Apart from the \texttt{valid} and \texttt{ready} flags present in the
Buffer-Producer model, only the additional \texttt{bytes\_left} integer is
required in the control bus. Although this field in the bus is an integer and
can indicate the actual number of remaining bytes, the CP protocol specifies
that it is to be treated as a boolean.
...
\begin{itemize}
	...
	\item If the consumer deasserts \texttt{ready}, it will not read the
		data in the bus.
